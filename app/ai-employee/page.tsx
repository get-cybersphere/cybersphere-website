...
const ACCORDION_ITEMS: AccordionItem[] = [
  {
    ...
    description:
      "Every call, text, and web chat answered instantly — 24/7.",
    chat: {
  ...
  },
  {
    ...
    description:
      "Responds in under 60 seconds — 45% higher conversion rate.",
    chat: {
  ...
  },
  {
    ...
    description:
      "Re-engages past customers at the right time to drive repeat business.",
    chat: {
  ...
  },
  {
    ...
    description:
      "Auto-requests reviews and responds on Google to boost rankings.",
    chat: {
...
function ChatBubble({ user, ai }: { user: string; ai: string }) {
  return (
    <motion.div
      initial={{ opacity: 0 }}
      animate={{ opacity: 1 }}
    ...
    >
      ...
      >
        ...
        <div className="bg-bg-gray rounded-2xl rounded-tl-sm px-4 py-3 max-w-[340px]">
          <p className="font-sans text-text-primary text-sm font-light leading-relaxed">
            {user}
          </p>
        </div>
      </motion.div>
...
const REVIEWS: ReviewData[] = [
  {
    ...
    initials: "MS",
    review:
      "Booked my first appointment in under a minute. The follow-up reminder was a nice touch — felt like a real person.",
    reply:
      "Thank you, Maria! We're glad the experience felt seamless. Looking forward to your next visit!",
  },
  {
    name: "David Chen",
    initials: "DC",
    review:
      "Called after hours expecting voicemail. Got everything handled on the spot — pricing, availability, appointment. Impressed.",
    reply:
      "Thanks for the kind words, David! Our team is available around the clock to help.",
  },
  {
    name: "Rachel Owens",
    initials: "RO",
    review:
      "They rescheduled my appointment without any hassle. Quick, polite, and super efficient. Five stars.",
    reply:
      "We appreciate the review, Rachel! Easy rescheduling is something we take pride in.",
  },
];
    ...
    <AnimatePresence mode="wait">
      ...
      >
        ...
        >
          ...
          <span className="font-heading text-text-primary text-base font-medium">
            {review.name}
          </span>
        </motion.div>
          ...
          <div className="flex items-start gap-3">
            ...
            <div>
              <div className="flex items-center gap-2 mb-1">
                <span className="font-heading text-text-primary text-sm font-medium">
                  AI Assistant
                </span>
...
const INBOX_ITEMS: InboxItem[] = [
  {
    icon: "\u{1F310}",
    bg: "bg-[#2d2d2d]",
    name: "New Lead — Website",
    preview:
      "Hi, I saw your services online. Do you have availability this week for a consultation?",
  },
  {
    icon: "\u2709\uFE0F",
    bg: "bg-[#7c8a7c]",
    name: "Maria Santos — Email",
    preview:
      "Hello, I need a quote for recurring office cleaning. Can someone get back to me today?",
  },
  {
    icon: "\u{1F4F1}",
    bg: "bg-blue/15",
    name: "David Chen — SMS",
    preview:
      "Hey, I missed my appointment yesterday. Any chance I can reschedule for tomorrow?",
  },
  {
    icon: "\u{1F4AC}",
    bg: "bg-[#6b8f8f]",
    name: "Rachel O. — Instagram",
    preview:
      "Do you guys offer weekend appointments? I can't make it during the week.",
  },
  {
    icon: "\u{1F4DE}",
    bg: "bg-amber-100",
    name: "Missed Call — (415) 882-1947",
    preview:
      "Voicemail: Looking to book a deep clean for my 2-bedroom apartment before move-out.",
  },
  {
    icon: "\u{1F310}",
    bg: "bg-[#2d2d2d]",
    name: "New Lead — Google Ads",
    preview:
      "I clicked on your ad for AC repair. What's your availability and pricing?",
  },
  {
    icon: "\u2709\uFE0F",
    bg: "bg-[#7c8a7c]",
    name: "Tom Bradley — Email",
    preview:
      "Following up on the estimate you sent last week. Ready to move forward if the slot's still open.",
  },
  {
    icon: "\u{1F4AC}",
    bg: "bg-[#6b8f8f]",
    name: "Lisa Park — Facebook",
    preview:
      "Hi! We spoke last month about catering for a birthday. Is June 14th still available?",
  },
];
...
const LEAD_CONVERSATION: ConversationMessage[] = [
  { from: "user", text: "How much for a full AC inspection?" },
  {
    from: "ai",
    text: "Our standard inspection is $89. If any repairs are needed, we apply that toward the service cost.",
  },
  { from: "user", text: "That's more than I expected honestly." },
  {
    from: "ai",
    text: "Totally understand. Most clients save $200\u2013$400 by catching small issues early. We also have a $59 tune-up if you'd prefer to start there.",
  },
  { from: "user", text: "Oh okay, the tune-up sounds good actually." },
  {
    from: "ai",
    text: "Great choice. I have tomorrow at 9 AM or Thursday at 2 PM open. Which works better?",
  },
  { from: "user", text: "Tomorrow 9 AM works." },
  {
    from: "ai",
    text: "You're all set for tomorrow at 9 AM. You'll get a confirmation text shortly. Anything else I can help with?",
  },
];
      ...
      <span
        className={
          displayed.length < text.length
            ? "inline-block w-[2px] h-[13px] bg-blue/40 ml-0.5 animate-pulse align-middle"
            : "hidden"
        }
      />
    </p>
...
function ChatMessage({
  ...
  const [phase, setPhase] = useState<"dots" | "typing" | "done">(
    isLatest ? "dots" : "done"
  );
  ...
  return (
    ...
    >
      <div
        className={`w-8 h-8 rounded-full flex items-center justify-center flex-shrink-0 mt-0.5 ${isAi ? "bg-blue/10" : "bg-bg-gray"}`}
      >
        {isAi ? (
      ...
      <div
        className={`rounded-2xl px-3.5 py-2.5 max-w-[300px] ${
          isAi
      ...
      >
        {phase === "dots" ? (
          ...
          <AITypedText
            text={msg.text}
            onDone={onTypingDone}
            className="font-sans text-text-primary text-[13px] font-light leading-relaxed"
          />
        ) : (
          <p className="font-sans text-text-primary text-[13px] font-light leading-relaxed">
            {msg.text}
          </p>
        )}
      </div>
...
function ConvertLeadChat() {
  ...
  return (
    <div
      ref={scrollRef}
      className="w-full max-w-[440px] mx-auto overflow-y-auto space-y-3 pr-1"
      style={{ maxHeight: 420 }}
    >
...
interface InboxCardItem extends InboxItem {
  id: number;
}
...
function InboxCardComponent({
  ...
  return (
    <div
      className={`rounded-xl border bg-white px-4 py-3.5 flex items-start gap-3 shadow-sm ${isNew ? "border-blue/30" : "border-[#d0d0d0]"}`}
    >
      <div
        className={`w-10 h-10 rounded-xl ${item.bg} flex items-center justify-center flex-shrink-0 text-lg`}
      >
        {item.icon}
      </div>
      <div className="min-w-0 flex-1">
        <div className="flex items-center gap-2">
          <p className="font-heading text-text-primary text-sm font-medium leading-tight truncate">
            {item.name}
          </p>
          {isNew && (
            <span className="flex-shrink-0 w-2 h-2 rounded-full bg-blue" />
          )}
        </div>
        <p className="text-text-body text-xs leading-relaxed mt-0.5 line-clamp-2">
          {item.preview}
        </p>
      </div>
    </div>
...
function InboxFeed() {
  const [feed, setFeed] = useState<InboxCardItem[]>(() =>
    ...
    <div ref={ref} className="w-full max-w-[400px] mx-auto">
      ...
      >
        ...
        <div className="flex items-center gap-2 ml-10.5">
          ...
          <span className="text-blue text-xs font-medium border border-blue/20 bg-blue/5 rounded-full px-2.5 py-0.5">
            Past clients
          </span>
        </div>
        <div className="flex items-center gap-2 ml-10.5 mt-2">
          ...
          <ChevronDown
            className="w-3 h-3 text-text-muted ml-auto rotate-180"
            strokeWidth={1.5}
          />
        </div>
      </motion.div>
      ...
      >
        Hi Sarah, this is your AI assistant at Renewal Spa. It&apos;s been 3
        months since your last visit — ready to book your next session? We
        have openings this Friday at 2 PM.
      </motion.div>
    </div>
  );
}
            ...
            >
              <div className="flex items-center justify-between gap-3 relative z-10">
                <div className="flex items-center gap-4">
                  ...
                  <span
                    className={`font-heading font-bold uppercase text-[18px] md:text-[20px] leading-[1.1] tracking-[-0.01em] ${
              ...
              >
                <p
                  className={`text-[12px] font-medium uppercase tracking-[0.06em] leading-[1.6] mt-4 max-w-[300px] ${
              ...
              {i === 0 && (
                <img
                  src="/images/backgrounds/offering1.png"
                  alt=""
                  className="absolute bottom-0 right-0 w-[40%] max-w-[160px] h-auto pointer-events-none opacity-80"
                />
              )}
              {i === 3 && (
                <img
                  src="/images/backgrounds/offering2.png"
                  alt=""
                  className="absolute top-0 right-0 w-[40%] max-w-[160px] h-auto pointer-events-none opacity-80"
                />
              )}
            </div>
          );
...
const INDUSTRIES: IndustryItem[] = [
  {
    icon: "https://newo.ai/wp-content/uploads/2026/02/Vector.svg",
    title: "Restaurant",
    description: "\u00A0Table bookings, catering, delivery, private dining",
  },
  {
    icon: "https://newo.ai/wp-content/uploads/2026/02/Vector-1.svg",
    title: "Catering",
    description: "Event quotes, menu customization, scheduling",
  },
  {
    icon: "https://newo.ai/wp-content/uploads/2026/02/icon-image-11.svg",
    title: "Hospitality / Hotels",
    description: "Reservations, concierge, multilingual support",
  },
  {
    icon: "https://newo.ai/wp-content/uploads/2026/02/Vector-2.svg",
    title: "Cleaning Services",
    description: "Instant quotes, recurring bookings, ZIP verification",
  },
  {
    icon: "https://newo.ai/wp-content/uploads/2026/02/icon-image-5.svg",
    title: "HVAC / Plumbing",
    description:
      "Emergency dispatch, appointment scheduling, estimates",
  },
  {
    icon: "https://newo.ai/wp-content/uploads/2026/02/Vector-3.svg",
    title: "Home Service Companies",
    description: "Multi-trade coordination, technician routing",
  },
  {
    icon: "https://newo.ai/wp-content/uploads/2026/02/icon-image-6-1.svg",
    title: "Healthcare",
    description:
      "Patient intake, insurance verification, appointment reminders",
  },
  {
    icon: "https://newo.ai/wp-content/uploads/2026/02/icon-image-7.svg",
    title: "Dental & Orthodontics",
    description:
      "New patient scheduling, treatment coordination, recall",
  },
  {
    icon: "https://newo.ai/wp-content/uploads/2026/02/icon-image-8.svg",
    title: "Fitness, Wellness, Beauty",
    description: "Membership sales, class booking, upsells",
  },
  {
    icon: "https://newo.ai/wp-content/uploads/2026/02/icon-image-9.svg",
    title: "Legal & Law Firms",
    description:
      "Lead qualification, consultation scheduling, intake",
  },
  {
    icon: "https://newo.ai/wp-content/uploads/2026/02/icon-image-10.svg",
    title: "And many more\u2026",
    description: "",
  },
];
      ...
      <section className="min-h-screen bg-white rounded-[20px] relative overflow-hidden flex flex-col">
        <div className="absolute inset-0 flex items-center justify-center md:items-center md:justify-end md:pr-[2%] md:pt-[5%] pointer-events-none">
          <div className="hero-enter hero-enter-d1">
            <img
              src="/images/backgrounds/employee.png"
              alt=""
              className="hero-float w-[min(620px,80vw)] h-auto rotate-0"
            />
          </div>
        </div>
            ...
            <div className="hidden lg:flex flex-col gap-8">
              <div>
                ...
                <p className="text-[13px] text-text-body leading-relaxed max-w-[180px]">
                  Replaces repetitive admin, CRM & scheduling tasks
                </p>
              </div>
              <div>
                <p className="text-[11px] font-medium uppercase tracking-[0.15em] text-text-muted mb-1">
                ...
                <p className="text-[13px] text-text-body leading-relaxed max-w-[180px]">
                  Precision execution with no manual mistakes
                </p>
              </div>
            </div>
        ...
        <div className="relative z-10 max-w-[1376px] w-full mx-auto px-4 md:px-8 pb-10 md:pb-16">
          <div className="hero-enter hero-enter-d2">
            ...
            >
              Your Team
              <br />
              <span className="text-blue">That Never</span>
              <br />
              Sleeps
            </h1>
          </div>
        ...
        <FadeUp>
          <div className="max-w-[1376px] mx-auto px-4 md:px-8 mb-12">
            ...
            <p className="text-text-body text-lg max-w-3xl leading-relaxed">
              Pre-configured{" "}
              <strong className="text-text-primary">AI Employees</strong> for
              10 core industries — and dozens more.
              <br />
