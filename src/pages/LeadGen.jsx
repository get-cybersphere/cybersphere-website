...
import leadgenBg from '../assets/images/background/leadgen.png'
...
function LeadProfileCard({ className = '' }) {
  return (
    <div className={`bg-white border-[1.5px] border-[#e5e5e5] rounded-xl p-5 shadow-sm ${className}`}>
      ...
      <div className="space-y-2.5">
        {[
          ...
          <div key={row.text} className="flex items-center gap-2 text-[11px] text-text-body">
            <row.icon className="w-3.5 h-3.5 text-text-muted flex-shrink-0" strokeWidth={1.5} />
            <span>{row.text}</span>
            {row.extra && (
              <span className="text-blue text-[10px] font-medium">{row.extra}</span>
            )}
            <ChevronDown className="w-3 h-3 text-text-muted ml-auto flex-shrink-0" strokeWidth={1.5} />
          </div>
        ))}
      </div>
...
function AcquisitionGraph() {
  ...
  const computePaths = useCallback(() => {
    ...
    // Left nodes -> card left edge
    leftRefs.current.forEach((el, i) => {
      if (!el) return
      const r = el.getBoundingClientRect()
      const startX = r.right - box.left        // right edge of node
      const startY = r.top - box.top + r.height / 2  // vertical center
      const endX = cx                           // card left edge
      const endY = cy + (ch * (i + 0.5)) / LEFT_NODES.length  // distributed on card edge
      const chanX = endX - 18 - i * 12         // staggered vertical channel
      result.push({
    ...
    // Right nodes -> card right edge
    rightRefs.current.forEach((el, i) => {
      if (!el) return
      const r = el.getBoundingClientRect()
      const startX = r.left - box.left          // left edge of node
      const startY = r.top - box.top + r.height / 2
      const endX = cx + cw                      // card right edge
  ...
  return (
    <div ref={wrapRef} className="rounded-2xl border-[1.5px] border-[#e5e5e5] bg-white p-6 md:p-10 overflow-hidden">
      ...
      <div className="hidden lg:block">
        <div ref={containerRef} className="relative" style={{ aspectRatio: '5 / 3' }}>
          {/* SVG — pixel-space, no viewBox, matches container exactly */}
          <svg className="absolute inset-0 w-full h-full pointer-events-none overflow-visible">
            {paths.map((p, i) => (
              <motion.path
                ...
                transition={{ delay: 0.2 + p.index * 0.08, duration: 0.6, ease: 'easeOut' }}
              />
            ))}
          </svg>
...
/* ── Pipeline Card (second horizontally-scrolled card) ── */

// Grid order is zigzag: row 1 L→R, row 2 R→L, row 3 L→R
// So middle row has steps swapped: [04] [03] instead of [03] [04]
const PIPELINE_STEPS = [
  ...
  // Row 2: right → left (swapped so 03 sits under 02)
  {
    num: '04',
    title: 'Outreach',
    rows: [
  ...
  // Row 3: left → right (05 sits under 04)
  {
    num: '05',
    title: 'Engage',
    rows: [
...
function PipelineCard() {
  ...
  const computePaths = useCallback(() => {
    ...
    // Use offset properties instead of getBoundingClientRect
    // so positions aren't affected by framer-motion transforms
    ...
    // Zigzag: 01→02 right, 02↓03 down, 03→04 left, 04↓05 down, 05→06 right
    // Grid positions: [0]=01, [1]=02, [2]=04, [3]=03, [4]=05, [5]=06
    const conns = [
      [0, 1, 'hr'],  // 01→02: horizontal right
      [1, 3, 'vd'],  // 02→03: vertical down (right column)
      [3, 2, 'hl'],  // 03→04: horizontal left
      [2, 4, 'vd'],  // 04→05: vertical down (left column)
      [4, 5, 'hr'],  // 05→06: horizontal right
    ]
  ...
  return (
    <div ref={ref} className="rounded-2xl border-[1.5px] border-[#e5e5e5] bg-white p-6 md:p-10 overflow-hidden">
      <div className="mb-6 md:mb-8">
        <h3 className="font-heading text-text-primary text-2xl md:text-3xl font-light">
          Pipeline
        </h3>
          ...
          {PIPELINE_STEPS.map((step, i) => (
            ...
            >
              ...
              <div className="flex items-center gap-2.5 mb-3">
                <span className="text-blue font-mono text-[11px] font-semibold">{step.num}</span>
                <span className="text-text-primary font-heading text-[14px] font-medium">{step.title}</span>
              </div>
              {/* Rows */}
      ...
      <FadeUp className="max-w-[1376px] mx-auto px-4 md:px-8 pb-16 md:pb-24">
        ...
        <div className="flex items-center justify-between mb-8">
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              onClick={() => scrollTo(Math.min(CARD_LABELS.length - 1, active + 1))}
              className="w-10 h-10 rounded-full border border-[#e5e5e5] flex items-center justify-center text-text-primary hover:border-[#111] hover:bg-[#fafafa] transition-all"
            >
      ...
      <section className="min-h-screen bg-white rounded-[20px] relative overflow-hidden flex flex-col">
        ...
        <div className="absolute inset-0 flex items-end justify-center md:items-center md:justify-end md:pr-[8%] md:pt-[10%] pointer-events-none">
          <div className="hero-enter hero-enter-d1">
            <img src={leadgenBg} alt="" className="hero-float w-[min(440px,55vw)] h-auto rotate-0" />
          </div>
        </div>
        ...
        <div className="relative z-10 max-w-[1376px] w-full mx-auto px-4 md:px-8 pt-24 md:pt-28 hero-enter">
          <div className="flex justify-between items-start">
            <div className="hidden lg:flex flex-col gap-8">
              <div>
                <p className="text-[11px] font-medium uppercase tracking-[0.15em] text-text-muted mb-1">Multi-Channel</p>
                <p className="text-[13px] text-text-body leading-relaxed max-w-[180px]">Capture from web, ads, social & referrals in one pipeline</p>
              </div>
              <div>
                <p className="text-[11px] font-medium uppercase tracking-[0.15em] text-text-muted mb-1">Instant Follow-Up</p>
                <p className="text-[13px] text-text-body leading-relaxed max-w-[180px]">Text, email & call sequences triggered in seconds</p>
              </div>
            </div>
        ...
        <div className="relative z-10 max-w-[1376px] w-full mx-auto px-4 md:px-8 pb-10 md:pb-16">
          <div className="hero-enter hero-enter-d2">
            ...
            >
              More Leads<br />
              <span className="text-blue">Less</span><br />
              Work
            </h1>
          </div>
